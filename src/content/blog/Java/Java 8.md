---
title: 'Java 8'
description: 'Java 8 features: lambdas, functional interfaces, Stream API, and Optional.'
pubDate: 2026-07-14
tags: ['java']
---
# Java 8
...
- decouple business logic, e.g. create a `ValidationService` that takes a `Predicate<User>` as an argument without changing the service code, following the open-closed principle
...
`Comparator` is a class or Lambda used to define multiple, custom sorting sequences without modifying the original class.
...
It takes one element, transforms it into a Stream of element, and then flattens all those individual streams into a single, continuous stream.
