---
title: 'Spring'
description: 'The Spring ecosystem: Spring vs. Spring Boot, auto-configuration, and starters.'
pubDate: 2026-07-23
tags: ['java', 'spring']
---
# Spring
...
- maintainability: You don't need to manually passing objects, the Container handles the wiring.
...
- easier integration with Spring's additional features
...
c. CSRF protection: generates a random, unique CSRF token for every session. For any state-changing request, the client must include this token. The server compares the token in the request with the token in the session.
...
- the client then clears all local storage and force-redirects the user to login page
